...
namespace Scanlab.Sirius
{
      ...
      IRtcAutoLaserControl
    {
        ...
        protected readonly uint RTC6_LIST_BUFFER_MAX = 10000;
        protected readonly uint RTC6_LIST3_BUFFER_SIZE = (uint)Math.Pow(2.0, 17.0);
        ...
        protected float laserPowerXValue;
        ...
        private bool isListAlcByVectorBegin;
        ...
        public Rtc6()
        {
            ...
            this.frequency = this.pulseWidth = this.laserPowerXValue = 0.0f;
        ...
        public Rtc6(uint index, string outputFileName = "")
          : this()
        {
            this.Index = index;
        ...
        public virtual bool Initialize(float kFactor, LaserMode laserMode, string ctbFileName)
        {
            ...
            if (!RTC6Wrap.Initialized)
            {
                uint num = RTC6Wrap.init_rtc6_dll();
                if (num != 0U)
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: fail to init_rtc6_dll = {1}", (object)this.Index, (object)num), Array.Empty<object>());
                RTC6Wrap.Initialized = num == 0U;
            }
            RTC6Wrap.n_stop_execution(this.Index + 1U);
            uint num1 = RTC6Wrap.n_load_program_file(this.Index + 1U, string.Empty);
            if (num1 != 0U)
                Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: fail to load_program_file = {1}", (object)this.Index, (object)num1), Array.Empty<object>());
            if (num1 != 0U)
            {
                switch ((int)num1 - 1)
                {
                    case 0:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: file error (file corrupt or incomplete)", (object)this.Index), Array.Empty<object>());
                        break;
                    case 1:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: memory error (DLL-internal, WINDOWS system memory)", (object)this.Index), Array.Empty<object>());
                        break;
                    case 2:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: File open error (empty string summitted or file not found)", (object)this.Index), Array.Empty<object>());
                        break;
                    case 3:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: DSP memory error", (object)this.Index), Array.Empty<object>());
                        break;
                    case 4:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: PCI download error (driver error)", (object)this.Index), Array.Empty<object>());
                        break;
                    case 7:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: System driver not found", (object)this.Index), Array.Empty<object>());
                        break;
                    case 9:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: Parameter error (incorrect No.)", (object)this.Index), Array.Empty<object>());
                        break;
                    case 10:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: Access error : board reserved for another application", (object)this.Index), Array.Empty<object>());
                        break;
                    case 11:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: Warning : 3D table or D3 selected but 3D option is not enabled.", (object)this.Index), Array.Empty<object>());
                        break;
                    case 12:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: Busy error: no download, board is BUSY or INTERNAL BUSY", (object)this.Index), Array.Empty<object>());
                        break;
                    case 13:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: PCI upload error (driver error, only applicable for download verification)", (object)this.Index), Array.Empty<object>());
                        break;
                    case 14:
                        Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: Verify error (only applicable for download verification)", (object)this.Index), Array.Empty<object>());
                        break;
                }
                return false;
            }
            uint Code = RTC6Wrap.n_get_last_error(this.Index + 1U);
            if (Code != 0U)
                RTC6Wrap.n_reset_error(this.Index + 1U, Code);
            int num2 = (int)RTC6Wrap.rtc6_count_cards();
            int num3 = (int)RTC6Wrap.get_dll_version();
            int num4 = (int)RTC6Wrap.get_hex_version();
            uint num5 = RTC6Wrap.get_rtc_version();
            uint num6 = RTC6Wrap.n_get_serial_number(this.Index + 1U);
            this.IsMOTF = Convert.ToBoolean(num5 & 256U);
            this.Is2ndHead = Convert.ToBoolean(num5 & 512U);
            this.Is3D = Convert.ToBoolean(num5 & 1024U);
            this.IsScanAhead = Convert.ToBoolean(num5 & 4096U);
            this.IsUFPM = Convert.ToBoolean(num5 & 8192U);
            this.IsSyncAxis = Convert.ToBoolean(num5 & 16384U);
            RTC6Wrap.n_set_laser_control(this.Index + 1U, 256U);
            ...
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: serial no = {1} initialized", (object)this.Index, (object)num6), Array.Empty<object>());
            return true;
        }
        ...
        {
            if (this.CtlGetStatus(RtcStatus.Busy) || tableIndex == CorrectionTableIndex.None)
                return false;
            switch (RTC6Wrap.n_load_correction_file(this.Index + 1U, ctbFileName, (uint)(tableIndex + 1), this.Is3D ? 3U : 2U))
            {
                case 0:
                    this.ctbFileName[(int)tableIndex] = ctbFileName;
                    Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: correction file loaded : {1} at {2}", (object)this.Index, (object)ctbFileName, (object)tableIndex.ToString()), Array.Empty<object>());
                    return true;
                case 1:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: file error (file corrupt or incomplete)", (object)this.Index), Array.Empty<object>());
                    break;
          ...
          CorrectionTableIndex secondaryHeadTableIndex = CorrectionTableIndex.None)
        {
            if (!this.Is3D)
            {
                RTC6Wrap.n_select_cor_table(this.Index + 1U, (uint)(primaryHeadTableIndex + 1), (uint)(secondaryHeadTableIndex + 1));
            ...
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: correction file selected : primary head= {1}. secondary head= {2}", (object)index, (object)str1, (object)str2), Array.Empty<object>());
            return true;
        }
        ...
        {
            RTC6Wrap.n_laser_signal_on(this.Index + 1U);
            this.isManualOn = true;
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: laser is on by manually", (object)this.Index), Array.Empty<object>());
            return true;
        }
        ...
        {
            RTC6Wrap.n_laser_signal_off(this.Index + 1U);
            this.isManualOn = false;
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: laser is off by manually", (object)this.Index), Array.Empty<object>());
            return true;
        }
        ...
        public bool CtlMove(Vector2 vPosition)
        {
            ...
            int int32_1 = Convert.ToInt32(vector2.X * this.KFactor);
            int int32_2 = Convert.ToInt32(vector2.Y * this.KFactor);
            RTC6Wrap.n_goto_xy(this.Index + 1U, int32_1, int32_2);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: scanner moved : {1}, {2}", (object)this.Index, (object)vector2.X, (object)vector2.Y), Array.Empty<object>());
            return true;
        }
        ...
        public bool CtlFrequency(float frequency, float pulseWidth)
        {
            ...
            RTC6Wrap.n_set_start_list(this.Index + 1U, 1U);
            RTC6Wrap.n_set_laser_timing(this.Index + 1U, (uint)(num * 64.0), (uint)((double)pulseWidth * 64.0), (uint)((double)pulseWidth * 64.0), 0U);
            RTC6Wrap.n_set_end_of_list(this.Index + 1U);
            RTC6Wrap.n_execute_list(this.Index + 1U, 1U);
          ...
          float scannerPolygon)
        {
            if (this.CtlGetStatus(RtcStatus.Busy))
                return false;
            RTC6Wrap.n_set_start_list(this.Index + 1U, 1U);
            RTC6Wrap.n_set_scanner_delays(this.Index + 1U, (uint)((double)scannerJump / 10.0), (uint)((double)scannerMark / 10.0), (uint)((double)scannerPolygon / 10.0));
            RTC6Wrap.n_set_laser_delays(this.Index + 1U, (int)((double)laserOn * 64.0), (uint)((double)laserOff * 64.0));
            RTC6Wrap.n_set_end_of_list(this.Index + 1U);
            RTC6Wrap.n_execute_list(this.Index + 1U, 1U);
            this.CtlBusyWait();
            return true;
        ...
        public bool CtlSpeed(float jump, float mark)
        {
            ...
            if (Speed2 > 800000.0)
                Speed2 = 800000.0;
            RTC6Wrap.n_set_jump_speed_ctrl(this.Index + 1U, Speed1);
            RTC6Wrap.n_set_mark_speed_ctrl(this.Index + 1U, Speed2);
            return true;
        }
        ...
        public bool CtlWriteData<T>(ExtensionChannel ch, T value, ICompensator<T> compensator = null)
        {
            ...
            switch (ch)
            {
                case ExtensionChannel.ExtDO2:
                    ...
                    RTC6Wrap.n_set_laser_pin_out(this.Index + 1U, Pins);
                    break;
                case ExtensionChannel.ExtDO8:
                    uint num1 = (uint)Convert.ChangeType((object)value, typeof(uint));
                    RTC6Wrap.n_write_8bit_port(this.Index + 1U, num1);
                    break;
                case ExtensionChannel.ExtDO16:
                    uint num2 = (uint)Convert.ChangeType((object)value, typeof(uint));
                    RTC6Wrap.n_write_io_port(this.Index + 1U, num2);
                    break;
                case ExtensionChannel.ExtAO1:
                    float num3 = (float)Convert.ChangeType((object)value, typeof(float));
                    uint num4 = (uint)((Math.Pow(2.0, 12.0) - 1.0) * (double)num3 / 10.0);
                    RTC6Wrap.n_write_da_x(this.Index + 1U, 1U, num4);
                    break;
                case ExtensionChannel.ExtAO2:
                    float num5 = (float)Convert.ChangeType((object)value, typeof(float));
                    uint num6 = (uint)((Math.Pow(2.0, 12.0) - 1.0) * (double)num5 / 10.0);
                    RTC6Wrap.n_write_da_x(this.Index + 1U, 2U, num6);
                    break;
                default:
                    return false;
            }
        ...
        public bool CtlWriteExtDO16(ushort bitPosition, bool onOff)
        {
            ...
            RTC6Wrap.n_write_io_port_mask(this.Index + 1U, (uint)num2, (uint)num1);
        ...
        public bool CtlReadData<T>(ExtensionChannel ch, out T value, ICompensator<T> compensator = null)
        {
            ...
            switch (ch)
            {
                case ExtensionChannel.ExtDI2:
                    uint num1 = RTC6Wrap.n_get_laser_pin_in(this.Index + 1U);
                    value = (T)Convert.ChangeType((object)num1, typeof(T));
                    break;
                case ExtensionChannel.ExtDO16:
                    uint num2 = RTC6Wrap.n_get_io_status(this.Index + 1U);
                    value = (T)Convert.ChangeType((object)num2, typeof(T));
                    break;
                case ExtensionChannel.ExtDI16:
                    uint num3 = RTC6Wrap.n_read_io_port(this.Index + 1U);
        ...
        /// <returns></returns>
        public virtual string CtlGetErrMsg(uint errorCode)
        {
            if (errorCode == 0U)
                return "";
            uint num = RTC6Wrap.n_get_error(this.Index + 1U);
        ...
        /// <returns></returns>
        public virtual bool CtlGetStatus(RtcStatus s)
        {
            bool flag1 = false;
            bool flag2 = false;
            switch (s)
            {
                case RtcStatus.Busy:
                    uint Status;
                    uint Pos;
                    RTC6Wrap.n_get_status(this.Index + 1U, out Status, out Pos);
                    bool flag3 = flag2 | Convert.ToBoolean(Status & 1U) | Convert.ToBoolean(Status & 128U) | Convert.ToBoolean(Status & 32768U) | Convert.ToBoolean(Status & 8388608U);
                ...
                case RtcStatus.List1Busy:
                    flag1 = Convert.ToBoolean(RTC6Wrap.n_read_status(this.Index + 1U) & 15U);
                    break;
                case RtcStatus.List2Busy:
                    flag1 = Convert.ToBoolean(RTC6Wrap.n_read_status(this.Index + 1U) & 16U);
                    break;
                case RtcStatus.NoError:
                    int num1 = this.CtlGetStatus(RtcStatus.Aborted) ? 1 : 0;
                    bool flag4 = RTC6Wrap.n_get_last_error(this.Index + 1U) > 0U;
                    flag1 = num1 == 0 && !flag4;
                    break;
                ...
                case RtcStatus.PositionAckOK:
                    int num2 = (int)RTC6Wrap.n_get_head_status(this.Index + 1U, 1U);
                    flag1 = Convert.ToBoolean((uint)(num2 & 8)) & Convert.ToBoolean((uint)(num2 & 16));
                    break;
                case RtcStatus.PowerOK:
                    flag1 = Convert.ToBoolean(RTC6Wrap.n_get_head_status(this.Index + 1U, 1U) & 128U);
                    break;
                case RtcStatus.TempOK:
                    flag1 = Convert.ToBoolean(RTC6Wrap.n_get_head_status(this.Index + 1U, 1U) & 64U);
                    break;
            }
            return flag1;
        }
        ...
        public bool CtlBusyWait()
        {
            ...
            while (false | Convert.ToBoolean(Status & 1U) | Convert.ToBoolean(Status & 128U) | Convert.ToBoolean(Status & 32768U) | Convert.ToBoolean(Status & 8388608U));
            return true;
        }
        ...
        public bool CtlAbort()
        {
            ...
            this.stream?.Dispose();
            this.stream = (StreamWriter)null;
            Logger.Log(Logger.Type.Warn, string.Format("rtc6 [{0}]: trying to abort !", (object)this.Index), Array.Empty<object>());
        ...
        public bool ListBegin(ILaser laser, ListType listType = ListType.Single)
        {
            ...
            switch (listType)
            {
                case ListType.Single:
                    RTC6Wrap.n_config_list(this.Index + 1U, (uint)Math.Pow(2.0, 23.0) - this.RTC6_LIST3_BUFFER_SIZE, 0U);
                    break;
                case ListType.Auto:
                    RTC6Wrap.n_config_list(this.Index + 1U, this.RTC6_LIST_BUFFER_MAX * 2U, this.RTC6_LIST_BUFFER_MAX * 2U);
                    break;
            }
            RTC6Wrap.n_set_start_list(this.Index + 1U, this.listIndex);
            this.stream?.WriteLine("; LIST HAS BEGAN : " + DateTime.Now.ToString() + " with " + listType.ToString());
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: list has began ...", (object)this.Index), Array.Empty<object>());
            return true;
        }
        ...
        public bool ListSpeed(float jump, float mark)
        {
            ...
            if (!this.IsListReady(2U))
                return false;
            RTC6Wrap.n_set_jump_speed(this.Index + 1U, Speed1);
            RTC6Wrap.n_set_mark_speed(this.Index + 1U, Speed2);
        ...
        public bool ListWait(float msec)
        {
            ...
            if (count > 0U)
            {
                ...
                for (int index = 0; (long)index < (long)count; ++index)
                    RTC6Wrap.n_long_delay(this.Index + 1U, 100000U);
            }
            if ((double)num > 0.0)
            {
                if (!this.IsListReady(1U))
                    return false;
                RTC6Wrap.n_long_delay(this.Index + 1U, (uint)((double)num * 100.0));
            }
        ...
        public bool ListLaserOn(float msec)
        {
            ...
            RTC6Wrap.n_laser_on_list(this.Index + 1U, (uint)(num / 10.0));
        ...
        public bool ListLaserOn()
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(1U))
                return false;
            RTC6Wrap.n_laser_signal_on_list(this.Index + 1U);
        ...
        public bool ListLaserOff()
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(1U))
                return false;
            RTC6Wrap.n_laser_signal_off_list(this.Index + 1U);
        ...
        public bool ListJump(Vector2 vPosition, float rampFactor = 1f)
        {
            ...
            {
                RTC6Wrap.n_jump_abs(this.Index + 1U, X, Y);
                this.stream?.WriteLine(string.Format("JUMP_TO = {0:F3}, {1:F3}", (object)vector2.X, (object)vector2.Y));
            }
            else
            {
                RTC6Wrap.n_para_jump_abs(this.Index + 1U, X, Y, (uint)((double)this.valueByVector * (double)rampFactor));
        ...
        public bool ListMark(Vector2 vPosition, float rampFactor = 1f)
        {
            ...
            if (!this.isListAlcByVectorBegin)
            {
                RTC6Wrap.n_mark_abs(this.Index + 1U, X, Y);
        ...
        public bool ListArc(Vector2 vCenter, float sweepAngle)
        {
            ...
            for (int index = 0; index < num1; ++index)
                RTC6Wrap.n_arc_abs(this.Index + 1U, (int)((double)vector2.X * (double)this.KFactor), (int)((double)vector2.Y * (double)this.KFactor), (double)Math.Sign(sweepAngle) * -360.0);
            RTC6Wrap.n_arc_abs(this.Index + 1U, (int)((double)vector2.X * (double)this.KFactor), (int)((double)vector2.Y * (double)this.KFactor), -num2);
            double num3 = 0.0;
          ...
          float rotateAngle = 0.0f)
        {
            ...
            if (!this.IsListReady(2U))
                return false;
            RTC6Wrap.n_set_ellipse(this.Index + 1U, (uint)((double)majorHalf * (double)this.KFactor), (uint)((double)minorHalf * (double)this.KFactor), -(double)startAngle, -(double)sweepAngle);
            RTC6Wrap.n_mark_ellipse_abs(this.Index + 1U, (int)((double)vector2.X * (double)this.KFactor), (int)((double)vector2.Y * (double)this.KFactor), (double)rotateAngle);
        ...
        public bool ListWriteData<T>(ExtensionChannel ch, T value, ICompensator<T> compensator = null)
        {
            if (!this.IsListReady(2U))
                return false;
            RTC6Wrap.n_list_nop(this.Index + 1U);
            if (compensator != null)
            {
                T output;
                if (!compensator.Interpolate(value, out output))
            ...
            switch (ch)
            {
                case ExtensionChannel.ExtDO2:
                    uint Pins = (uint)Convert.ChangeType((object)value, typeof(uint));
                    RTC6Wrap.n_set_laser_pin_out_list(this.Index + 1U, Pins);
                    break;
                case ExtensionChannel.ExtDO8:
                    uint num1 = (uint)Convert.ChangeType((object)value, typeof(uint));
                    RTC6Wrap.n_write_8bit_port_list(this.Index + 1U, num1);
                    break;
                case ExtensionChannel.ExtDO16:
                    uint num2 = (uint)Convert.ChangeType((object)value, typeof(uint));
                    RTC6Wrap.n_write_io_port_list(this.Index + 1U, num2);
                    break;
                case ExtensionChannel.ExtAO1:
                    double num3 = (double)Convert.ChangeType((object)value, typeof(double));
                    double num4 = (Math.Pow(2.0, 12.0) - 1.0) * num3 / 10.0;
                    RTC6Wrap.n_write_da_x_list(this.Index + 1U, 1U, (uint)num4);
                    break;
                case ExtensionChannel.ExtAO2:
                    double num5 = (double)Convert.ChangeType((object)value, typeof(double));
                    double num6 = (Math.Pow(2.0, 12.0) - 1.0) * num5 / 10.0;
                    RTC6Wrap.n_write_da_x_list(this.Index + 1U, 2U, (uint)num6);
                    break;
                default:
                    return false;
            }
        ...
        public bool ListWriteExtDO16(ushort bitPosition, bool onOff)
        {
            ...
            ushort num1 = (ushort)(1U << (int)bitPosition);
            ushort num2 = onOff ? num1 : (ushort)0;
            RTC6Wrap.n_write_io_port_mask_list(this.Index + 1U, (uint)num2, (uint)num1);
            this.stream?.WriteLine(string.Format("DATA : ExtDO16 : BIT{0} : {1}", (object)bitPosition, (object)onOff.ToString()));
            return true;
        }
        ...
        public bool ListEnd()
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(2U))
                return false;
            RTC6Wrap.n_set_vector_control(this.Index + 1U, 0U, 0U);
            RTC6Wrap.n_set_end_of_list(this.Index + 1U);
            this.stream?.WriteLine("; LIST ENDED : " + DateTime.Now.ToString());
            this.stream?.WriteLine(Environment.NewLine);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: list has ended. counts= {1}", (object)this.Index, (object)this.listTotalCount), Array.Empty<object>());
            return true;
        }
        ...
        public bool ListExecute(bool busyWait = true)
        {
            ...
            {
                case ListType.Single:
                    RTC6Wrap.n_execute_list(this.Index + 1U, this.listIndex);
                    break;
                case ListType.Auto:
                    uint Status;
                    uint Pos;
                    RTC6Wrap.n_get_status(this.Index + 1U, out Status, out Pos);
                    if ((0 | (Convert.ToBoolean(Status & 1U) ? 1 : 0) | (Convert.ToBoolean(Status & 128U) ? 1 : 0) | (Convert.ToBoolean(Status & 32768U) ? 1 : 0) | (Convert.ToBoolean(Status & 8388608U) ? 1 : 0)) != 0)
                    {
                        RTC6Wrap.n_auto_change(this.Index + 1U);
                        break;
                    }
                    RTC6Wrap.n_execute_list(this.Index + 1U, this.listIndex);
                    break;
            }
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: list executing", (object)this.Index), Array.Empty<object>());
            if (busyWait)
                this.CtlBusyWait();
            if (this.CtlGetStatus(RtcStatus.Aborted))
                return false;
        ...
        protected bool IsListReady(uint count)
        {
            ...
            switch (this.listType)
            {
                case ListType.Single:
                    ...
                    return (double)this.listTotalCount < Math.Pow(2.0, 23.0) - (double)this.RTC6_LIST3_BUFFER_SIZE;
                case ListType.Auto:
                    if (this.listTempCount + count >= this.RTC6_LIST_BUFFER_MAX)
                    {
                        uint Status;
                        uint Pos;
                        RTC6Wrap.n_get_status(this.Index + 1U, out Status, out Pos);
                        ...
                        else
                        {
                            ...
                            switch (this.listIndex)
                            {
                                case 1:
                                    ...
                                    do
                                    {
                                        num1 = RTC6Wrap.n_read_status(this.Index + 1U);
                                        Thread.Sleep(10);
                                    }
                                ...
                                case 2:
                                    ...
                                    do
                                    {
                                        num2 = RTC6Wrap.n_read_status(this.Index + 1U);
                                        Thread.Sleep(10);
                                    }
        ...
        public bool CtlHomePosition(Vector2 vPosition)
        {
            ...
            RTC6Wrap.n_home_position(this.Index + 1U, XHome, YHome);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: home position: {1:F3}, {2:F3}", (object)this.Index, (object)vector2.X, (object)vector2.Y), Array.Empty<object>());
            return true;
        }
        ...
        public bool CtlFirstPulseKiller(float usec)
        {
            if (this.CtlGetStatus(RtcStatus.Busy))
                return false;
            RTC6Wrap.n_set_firstpulse_killer(this.Index + 1U, (uint)((double)usec * 64.0));
            this.Fpk = usec;
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: fpk= {1:F1} usec", (object)this.Index, (object)usec), Array.Empty<object>());
            return true;
        }
        ...
        {
            RTC6Wrap.n_set_laser_control(this.Index + 1U, laserControlSignal.ToUInt());
            return true;
        }
        ...
        {
            RTC6Wrap.n_set_control_mode(this.Index + 1U, mode.ToUInt());
            RTC6Wrap.n_set_max_counts(this.Index + 1U, maxStartCounts);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: external control= {1}, max= {2}", (object)this.Index, (object)mode.ToUInt(), (object)maxStartCounts), Array.Empty<object>());
            return true;
        }
        ...
        {
            counts = RTC6Wrap.n_get_counts(this.Index + 1U);
            return true;
        }
        ...
        public bool ListFirstPulseKiller(float usec)
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(1U))
                return false;
            RTC6Wrap.n_set_firstpulse_killer_list(this.Index + 1U, (uint)((double)usec * 64.0));
            this.Fpk = usec;
        ...
        public bool ListSkyWriting(float laserOnShift, float timeLag, float angularLimit)
        {
            ...
            {
                RTC6Wrap.n_set_sky_writing_mode_list(this.Index + 1U, 0U);
                RTC6Wrap.n_set_sky_writing_list(this.Index + 1U, 0.0, 0);
                double d = 1.57079631462693;
                RTC6Wrap.n_set_sky_writing_limit_list(this.Index + 1U, Math.Cos(d));
            }
            else
            {
                RTC6Wrap.n_set_sky_writing_mode_list(this.Index + 1U, 3U);
                RTC6Wrap.n_set_sky_writing_list(this.Index + 1U, (double)timeLag * 64.0, (int)((double)laserOnShift * 64.0));
        ...
        public bool ListPixelLine(float usec, Vector2 vDelta, uint pixelCount, ExtensionChannel ext = ExtensionChannel.ExtAO2)
        {
            ...
            {
                case ExtensionChannel.ExtDO8:
                    RTC6Wrap.n_set_pixel_line(this.Index + 1U, 3U, (uint)((double)usec / 2.0 * 64.0), (double)num1, (double)num2);
                    break;
                case ExtensionChannel.ExtDO16:
                    RTC6Wrap.n_set_pixel_line(this.Index + 1U, 4U, (uint)((double)usec / 2.0 * 64.0), (double)num1, (double)num2);
                    break;
                case ExtensionChannel.ExtAO1:
                    RTC6Wrap.n_set_pixel_line(this.Index + 1U, 1U, (uint)((double)usec / 2.0 * 64.0), (double)num1, (double)num2);
                    break;
                case ExtensionChannel.ExtAO2:
                    RTC6Wrap.n_set_pixel_line(this.Index + 1U, 2U, (uint)((double)usec / 2.0 * 64.0), (double)num1, (double)num2);
                    break;
                default:
                    Logger.Log(Logger.Type.Warn, string.Format("rtc6 [{0}]: {1} is not supported in ListPixelLine", (object)this.Index, (object)ext), Array.Empty<object>());
                    return false;
            }
        ...
        public bool ListPixel(float usec, float weight = 0.0f, ICompensator<float> compensator = null)
        {
            ...
            {
                case ExtensionChannel.ExtDO8:
                    int num1 = (int)(254.0 * (double)weight);
                    RTC6Wrap.n_set_pixel(this.Index + 1U, (uint)((double)usec * 64.0), (uint)num1);
                    break;
                case ExtensionChannel.ExtDO16:
                    int num2 = (int)(65534.0 * (double)weight);
                    RTC6Wrap.n_set_pixel(this.Index + 1U, (uint)((double)usec * 64.0), (uint)num2);
                    break;
                case ExtensionChannel.ExtAO1:
                case ExtensionChannel.ExtAO2:
                    uint PortOutValue2 = (uint)((Math.Pow(2.0, 12.0) - 1.0) * (double)weight);
                    RTC6Wrap.n_set_pixel(this.Index + 1U, (uint)((double)usec * 64.0), PortOutValue2);
                    break;
                default:
                    Logger.Log(Logger.Type.Warn, string.Format("rtc6 [{0}]: {1} is not supported in ListPixelLine", (object)this.Index, (object)this.pixelExtensionChannel), Array.Empty<object>());
                    return false;
            }
            return true;
        }
          ...
          ICompensator<float> analogCompensator = null)
        {
            ...
            {
                case ExtensionChannel.ExtAO1:
                    RTC6Wrap.n_set_pixel_line(this.Index + 1U, 1U, (uint)((double)periodUsec / 2.0 * 64.0), (double)num1, (double)num2);
                    break;
                case ExtensionChannel.ExtAO2:
                    RTC6Wrap.n_set_pixel_line(this.Index + 1U, 2U, (uint)((double)periodUsec / 2.0 * 64.0), (double)num1, (double)num2);
                    break;
                default:
                    Logger.Log(Logger.Type.Warn, string.Format("rtc6 [{0}]: {1} is not supported in ListPixelLine", (object)this.Index, (object)ext), Array.Empty<object>());
                    break;
            ...
            for (int index = 0; index < usecValues.Length; ++index)
            {
                ...
                uint PortOutValue2 = (uint)((Math.Pow(2.0, 12.0) - 1.0) * (double)input2 / 10.0);
                RTC6Wrap.n_set_pixel(this.Index + 1U, (uint)((double)input1 * 64.0), PortOutValue2);
            }
            return flag;
        }
        ...
        public bool ListWobbel(float amplitudeX, float amplitudeY, float frequencyHz)
        {
            ...
            RTC6Wrap.n_set_wobbel(this.Index + 1U, Transversal, Longitudinal, (double)this.frequency);
            return true;
        }
        ...
        public bool ListJumpHard(Vector2 vPosition, float laserOn, float laserOff)
        {
            ...
            if (!this.IsListReady(1U))
                return false;
            RTC6Wrap.n_micro_vector_abs(this.Index + 1U, X, Y, (int)laserOn * 2, (int)laserOff * 2);
            this.vPhysical = vector2;
            this.vLogical = vPosition;
        ...
        /// <returns></returns>
        public bool ListJumpHard(float x, float y, float laserOnUSec, float laserOffUSec) => this.ListJumpHard(new Vector2(x, y), laserOnUSec, laserOffUSec);
        ...
        public bool CtlGetMeasurement(MeasurementChannel channel, out int[] data)
        {
            ...
            uint Busy;
            uint Pos;
            RTC6Wrap.n_measurement_status(this.Index + 1U, out Busy, out Pos);
            if (Busy > 0U)
                return false;
            data = new int[(int)Pos];
            RTC6Wrap.n_get_waveform(this.Index + 1U, (uint)channel, Pos, data);
            return true;
        }
        ...
        public bool ListMeasurementBegin(float frequency, MeasurementChannel[] channels)
        {
            ...
            RTC6Wrap.n_set_trigger4(this.Index + 1U, (uint)(num / 10.0), (uint)channels[0], (uint)channels[1], (uint)channels[2], (uint)channels[3]);
            return true;
        }
        ...
        public bool ListMeasurementEnd()
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(2U))
                return false;
            RTC6Wrap.n_set_trigger(this.Index + 1U, 0U, 0U, 0U);
            RTC6Wrap.n_set_trigger4(this.Index + 1U, 0U, 0U, 0U, 0U, 0U);
            return true;
        }
          ...
          ICompensator<T> compensator = null)
        {
            ...
            {
                int num = RTC6Wrap.n_load_position_control(this.Index + 1U, (string)null, 0U);
                Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control by position dependent is disabled", (object)this.Index), Array.Empty<object>());
            }
            else
            {
                int num = RTC6Wrap.n_load_position_control(this.Index + 1U, this.AutoLaserControlByPositionFileName, this.AutoLaserControlByPositionTableNo);
                if (num <= -1 && num >= -50)
                {
                    Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control by position dependent: {1} [{2}]: {3} points", (object)this.Index, (object)this.AutoLaserControlByPositionFileName, (object)this.AutoLaserControlByPositionTableNo, (object)Math.Abs(num)), Array.Empty<object>());
                }
                else
                {
                    switch (num)
                    {
                        case 1:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: No valid data points found(though Table No found", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                        case 3:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: File not founded", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                        case 4:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: DSP memory error", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                        case 5:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: BUSY error, board was BUSY or INTERNAL-BUSY", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                        case 8:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: Board is locked by another user program", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                        case 11:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: PCI error", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                        case 13:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: The specified table number was not found in the file", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                        default:
                            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control: {1}: unknown error", (object)this.Index, (object)this.AutoLaserControlByPositionFileName), Array.Empty<object>());
                            break;
                    }
                    return false;
                }
          ...
          ICompensator<T> compensator = null)
        {
            ...
            switch (ctrl)
            {
                case AutoLaserControlSignal.Disabled:
                    num1 = RTC6Wrap.n_set_auto_laser_control(this.Index + 1U, (uint)ctrl, 0U, (uint)mode, 0U, 0U);
                    break;
                ...
                case AutoLaserControlSignal.Analog2:
                    ...
                    num1 = RTC6Wrap.n_set_auto_laser_control(this.Index + 1U, (uint)ctrl, num5, (uint)mode, MinValue1, MaxValue1);
                    break;
                case AutoLaserControlSignal.ExtDO8Bit:
                    ...
                    num1 = RTC6Wrap.n_set_auto_laser_control(this.Index + 1U, (uint)ctrl, num6, (uint)mode, MinValue2, MaxValue2);
                    break;
                case AutoLaserControlSignal.PulseWidth:
                    ...
                    num1 = RTC6Wrap.n_set_auto_laser_control(this.Index + 1U, (uint)ctrl, (uint)((double)num7 * 64.0), (uint)mode, (uint)((double)num8 * 64.0), (uint)((double)num9 * 64.0));
                    break;
                case AutoLaserControlSignal.Frequency:
                    ...
                    num1 = RTC6Wrap.n_set_auto_laser_control(this.Index + 1U, (uint)ctrl, (uint)(num10 * 64.0), (uint)mode, (uint)(num11 * 64.0), (uint)(num12 * 64.0));
                    break;
                case AutoLaserControlSignal.ExtDO16:
                    ...
                    num1 = RTC6Wrap.n_set_auto_laser_control(this.Index + 1U, (uint)ctrl, num13, (uint)mode, MinValue3, MaxValue3);
                    break;
            }
            if (mode == AutoLaserControlMode.Disabled)
                Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control : {1} disabled", (object)this.Index, (object)ctrl), Array.Empty<object>());
            switch (num1)
            {
                case 0:
                    Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: automatic laser control : {1}, {2}, 100%={3}, min={4}, max={5}, ret={6}", (object)this.Index, (object)mode, (object)ctrl, (object)percentage100, (object)min, (object)max, (object)num1), Array.Empty<object>());
                    return true;
                case 1:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: automatic laser control : no primary scan head active", (object)this.Index), Array.Empty<object>());
                    break;
                case 2:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: automatic laser control : no iDrive scan system", (object)this.Index), Array.Empty<object>());
                    break;
                case 3:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: automatic laser control : invalid {1} value", (object)this.Index, (object)ctrl), Array.Empty<object>());
                    break;
                case 4:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: automatic laser control : invalid {1} value", (object)this.Index, (object)mode), Array.Empty<object>());
                    break;
                case 5:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: automatic laser control : access denied", (object)this.Index), Array.Empty<object>());
                    break;
                default:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: automatic laser control : unknown error: {1}", (object)this.Index, (object)num1), Array.Empty<object>());
                    break;
            }
            return false;
        }
          ...
          ICompensator<T> compensator = null)
        {
            ...
            switch (ctrl)
            {
                ...
                case AutoLaserControlSignal.Analog2:
                    ...
                    RTC6Wrap.n_set_vector_control(this.Index + 1U, (uint)ctrl, num2);
                ...
                case AutoLaserControlSignal.ExtDO8Bit:
                    ...
                    RTC6Wrap.n_set_vector_control(this.Index + 1U, (uint)ctrl, num3);
                ...
                case AutoLaserControlSignal.PulseWidth:
                    ...
                    RTC6Wrap.n_set_vector_control(this.Index + 1U, (uint)ctrl, (uint)((double)num4 * 64.0));
                ...
                case AutoLaserControlSignal.Frequency:
                    ...
                    RTC6Wrap.n_set_vector_control(this.Index + 1U, (uint)ctrl, (uint)(num5 * 64.0));
                ...
                case AutoLaserControlSignal.ExtDO16:
                    ...
                    RTC6Wrap.n_set_vector_control(this.Index + 1U, (uint)ctrl, num6);
        ...
        public bool ListAlcByVectorEnd()
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(2U))
                return false;
            RTC6Wrap.n_set_vector_control(this.Index + 1U, (uint)this.byVectorAlcControlSignal, 0U);
            this.valueByVector = 0U;
        ...
        public bool CtlHeadOffset(ScannerHead head, Vector2 offset, float angle)
        {
            ...
            switch (head)
            {
                case ScannerHead.Primary:
                    RTC6Wrap.n_set_angle(this.Index + 1U, 1U, (double)angle, 1U);
                    RTC6Wrap.n_set_offset_xyz(this.Index + 1U, 1U, XOffset, YOffset, 0, 1U);
                    this.PrimaryHeadOffset = offset;
                    this.PrimaryHeadAngle = angle;
                    Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: primary head offset : {1}, angle={2:F3}", (object)this.Index, (object)offset.ToString(), (object)angle), Array.Empty<object>());
                    break;
                case ScannerHead.Secondary:
                    RTC6Wrap.n_set_angle(this.Index + 1U, 2U, (double)angle, 1U);
                    RTC6Wrap.n_set_offset_xyz(this.Index + 1U, 2U, XOffset, YOffset, 0, 1U);
                    this.SecondaryHeadOffset = offset;
                    this.SecondaryHeadAngle = angle;
                    Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: secondary head offset : {1}, angle={2:F3}", (object)this.Index, (object)offset.ToString(), (object)angle), Array.Empty<object>());
                    break;
                default:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: fail to reset head offset", (object)this.Index), Array.Empty<object>());
                    return false;
            }
            return true;
        }
        ...
        public bool ListHeadOffset(ScannerHead head, Vector2 offset, float angle)
        {
            ...
            {
                case ScannerHead.Primary:
                    RTC6Wrap.n_set_angle_list(this.Index + 1U, 1U, (double)angle, 1U);
                    RTC6Wrap.n_set_offset_xyz_list(this.Index + 1U, 1U, XOffset, YOffset, 0, 1U);
                    this.PrimaryHeadOffset = offset;
                    this.PrimaryHeadAngle = angle;
                    break;
                case ScannerHead.Secondary:
                    RTC6Wrap.n_set_angle_list(this.Index + 1U, 1U, (double)angle, 1U);
                    RTC6Wrap.n_set_offset_xyz_list(this.Index + 1U, 2U, XOffset, YOffset, 0, 1U);
                    this.SecondaryHeadOffset = offset;
                    this.SecondaryHeadAngle = angle;
                    break;
                default:
        ...
        public bool CtlMove(Vector3 vPosition)
        {
            ...
            RTC6Wrap.n_goto_xyz(this.Index + 1U, X, Y, Z);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: scanner moved : {1:F3}, {2:F3}, {3:F3} +{4:F3}", (object)this.Index, (object)vector3.X, (object)vector3.Y, (object)vector3.Z, (object)this.ZOffset), Array.Empty<object>());
        ...
        public bool CtlZOffset(float zOffset)
        {
            ...
            RTC6Wrap.n_set_offset_xyz(this.Index + 1U, 0U, 0, 0, ZOffset, 1U);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: z offset = {1:F3}", (object)this.Index, (object)zOffset), Array.Empty<object>());
            return true;
        }
        ...
        public bool CtlZDefocus(float zDefocus)
        {
            ...
            RTC6Wrap.n_set_defocus(this.Index + 1U, Shift);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: defocus = {1:F3}", (object)this.Index, (object)zDefocus), Array.Empty<object>());
            return true;
        }
        ...
        public bool ListZOffset(float zOffset)
        {
            ...
            if (!this.IsListReady(1U))
                return false;
            RTC6Wrap.n_set_offset_xyz_list(this.Index + 1U, 0U, 0, 0, ZOffset, 1U);
            return true;
        }
        ...
        public bool ListZDefocus(float zDefocus)
        {
            ...
            if (!this.IsListReady(1U))
                return false;
            RTC6Wrap.n_set_defocus_list(this.Index + 1U, Shift);
            return true;
        }
        ...
        public bool ListJump(Vector3 vPosition, float rampFactor = 1f)
        {
            ...
            if (!this.isListAlcByVectorBegin)
            {
                RTC6Wrap.n_jump_abs_3d(this.Index + 1U, X, Y, Z);
            ...
            {
                RTC6Wrap.n_para_jump_abs_3d(this.Index + 1U, X, Y, Z, (uint)((double)this.valueByVector * (double)rampFactor));
        ...
        public bool ListMark(Vector3 vPosition, float rampFactor = 1f)
        {
            ...
            if (!this.isListAlcByVectorBegin)
            {
                RTC6Wrap.n_mark_abs_3d(this.Index + 1U, X, Y, Z);
            ...
            {
                RTC6Wrap.n_para_mark_abs_3d(this.Index + 1U, X, Y, Z, (uint)((double)this.valueByVector * (double)rampFactor));
        ...
        public bool ListArc(Vector3 vCenter, float sweepAngle)
        {
            ...
            if (!this.IsListReady((uint)(num1 + 1)))
                return false;
            for (int index = 0; index < num1; ++index)
                RTC6Wrap.n_arc_abs_3d(this.Index + 1U, (int)((double)vector3.X * (double)this.KFactor), (int)((double)vector3.Y * (double)this.KFactor), (int)(((double)this.ZOffset + (double)vector3.Z) * (double)this.KZFactor), (double)Math.Sign(sweepAngle) * -360.0);
            RTC6Wrap.n_arc_abs_3d(this.Index + 1U, (int)((double)vector3.X * (double)this.KFactor), (int)((double)vector3.Y * (double)this.KFactor), (int)(((double)this.ZOffset + (double)vector3.Z) * (double)this.KZFactor), -num2);
            double num3 = 0.0;
            if ((double)this.vLogical.Y != (double)vCenter.Y || (double)this.vLogical.X != (double)vCenter.Y)
                num3 = Math.Atan2((double)this.vLogical.Y - (double)vCenter.Y, (double)this.vLogical.X - (double)vCenter.X);
            double num4 = Math.Sqrt(((double)vCenter.X - (double)this.vLogical.X) * ((double)vCenter.X - (double)this.vLogical.X) + ((double)vCenter.Y - (double)this.vLogical.Y) * ((double)vCenter.Y - (double)this.vLogical.Y));
        ...
        {
            RTC6Wrap.n_init_fly_2d(this.Index + 1U, (int)((double)this.EncXCountsPerMm * (double)offsetX), (int)((double)this.EncYCountsPerMm * (double)offsetY), 0U);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: motf encoder reset with offset {1:F3}, {2:F3}", (object)this.Index, (object)offsetX, (object)offsetY), Array.Empty<object>());
            return true;
        }
        ...
        public bool CtlEncoderSpeed(float encXSimulatedSpeed, float encYSimulatedSpeed)
        {
            if (0.0 == (double)encXSimulatedSpeed && 0.0 == (double)encYSimulatedSpeed)
                RTC6Wrap.n_simulate_encoder(this.Index + 1U, 0U);
            else if (0.0 != (double)encXSimulatedSpeed)
                RTC6Wrap.n_simulate_encoder(this.Index + 1U, 1U);
            else if (0.0 != (double)encYSimulatedSpeed)
                RTC6Wrap.n_simulate_encoder(this.Index + 1U, 2U);
            this.EncXSimulatedSpeed = encXSimulatedSpeed;
            this.EncYSimulatedSpeed = encYSimulatedSpeed;
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: motf encoder simulated with {1:F3}, {2:F3} mm/s", (object)this.Index, (object)encXSimulatedSpeed, (object)encYSimulatedSpeed), Array.Empty<object>());
            return true;
        }
        ...
        {
            RTC6Wrap.n_get_encoder(this.Index + 1U, out encX, out encY);
            encXmm = encYmm = 0.0f;
        ...
        {
            RTC6Wrap.n_set_fly_tracking_error(this.Index + 1U, xUsec / 10U, yUsec / 10U);
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: motf set tracking error xy= {1}, {2} usec", (object)this.Index, (object)xUsec, (object)yUsec), Array.Empty<object>());
            return true;
        }
        ...
        {
            int num = RTC6Wrap.n_load_fly_2d_table(this.Index + 1U, fileName, tableNo);
            bool flag = false;
            if (num < 0)
            {
                Logger.Log(Logger.Type.Warn, string.Format("rtc6 [{0}]: motf encoder compensate table reset/loaded: {1} with {2} points", (object)this.Index, (object)fileName, (object)Math.Abs(num)), Array.Empty<object>());
                flag = true;
            }
            switch (num)
            {
                case 0:
                    Logger.Log(Logger.Type.Warn, string.Format("rtc6 [{0}]: motf encoder compensate table reset", (object)this.Index), Array.Empty<object>());
                    flag = true;
                    break;
                case 1:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf encoder compensate table : no valid data points found", (object)this.Index), Array.Empty<object>());
                    break;
                case 2:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf encoder compensate table : out of memory", (object)this.Index), Array.Empty<object>());
                    break;
                case 3:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf encoder compensate table : file not found", (object)this.Index), Array.Empty<object>());
                    break;
                case 4:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf encoder compensate table : DSP memory error", (object)this.Index), Array.Empty<object>());
                    break;
                case 5:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf encoder compensate table : BUSY error", (object)this.Index), Array.Empty<object>());
                    break;
                case 8:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf encoder compensate table : RTC6 board is locked by another application", (object)this.Index), Array.Empty<object>());
                    break;
                case 13:
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf encoder compensate table : The specified table number was not found in the file.", (object)this.Index), Array.Empty<object>());
                    break;
            }
            return flag;
        }
        ...
        public bool CtlExternalControlDelay(RtcEncoder enc, float distance)
        {
            switch (enc)
            {
                case RtcEncoder.EncX:
                    RTC6Wrap.n_set_ext_start_delay(this.Index + 1U, (int)((double)this.EncXCountsPerMm * (double)distance), 0U);
                    break;
                case RtcEncoder.EncY:
                    RTC6Wrap.n_set_ext_start_delay(this.Index + 1U, (int)((double)this.EncYCountsPerMm * (double)distance), 1U);
                    break;
            }
            Logger.Log(Logger.Type.Info, string.Format("rtc6 [{0}]: motf external control delay distance= {1}, {2:F3} mm", (object)this.Index, (object)enc, (object)distance), Array.Empty<object>());
            return true;
        }
        ...
        public bool ListMOTFBegin(bool encoderReset = false)
        {
            ...
            else if ((double)this.EncXSimulatedSpeed == 0.0)
            {
                if (this.EncXCountsPerMm == 0)
                {
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf invalid encoder0 counts/mm value)", (object)this.Index), Array.Empty<object>());
                    return false;
                }
                ScaleX = (double)this.KFactor / (double)this.EncXCountsPerMm;
            ...
            else if ((double)this.EncYSimulatedSpeed == 0.0)
            {
                if (this.EncYCountsPerMm == 0)
                {
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf invalid encoder1 counts/mm value)", (object)this.Index), Array.Empty<object>());
                    return false;
                }
            ...
            else
            {
                ...
                {
                    Logger.Log(Logger.Type.Error, string.Format("rtc6 [{0}]: motf invalid encoder0,1 counts/mm value)", (object)this.Index), Array.Empty<object>());
                    return false;
                }
                ScaleX = (double)this.KFactor * (double)this.EncXSimulatedSpeed / 1000000.0;
                ScaleY = (double)this.KFactor * (double)this.EncYSimulatedSpeed / 1000000.0;
            ...
            if (!this.IsListReady(1U))
                return false;
            if (encoderReset)
                RTC6Wrap.n_set_fly_2d(this.Index + 1U, ScaleX, ScaleY);
            else
                RTC6Wrap.n_activate_fly_2d(this.Index + 1U, ScaleX, ScaleY);
            return true;
        }
        ...
        public bool ListExternalControlDelay(RtcEncoder enc, float distance)
        {
            ...
            {
                case RtcEncoder.EncX:
                    RTC6Wrap.n_set_ext_start_delay_list(this.Index + 1U, (int)((double)this.EncXCountsPerMm * (double)distance), 0U);
                    break;
                case RtcEncoder.EncY:
                    RTC6Wrap.n_set_ext_start_delay_list(this.Index + 1U, (int)((double)this.EncYCountsPerMm * (double)distance), 1U);
                    break;
            }
            return true;
        }
        ...
        public bool ListMOTFWait(RtcEncoder enc, float position, EncoderWaitCondition cond)
        {
            ...
            {
                case RtcEncoder.EncX:
                    RTC6Wrap.n_wait_for_encoder_mode(this.Index + 1U, (int)((double)position * (double)this.EncXCountsPerMm), 0U, (int)cond);
                    break;
                case RtcEncoder.EncY:
                    RTC6Wrap.n_wait_for_encoder_mode(this.Index + 1U, (int)((double)position * (double)this.EncXCountsPerMm), 1U, (int)cond);
                    break;
            }
            return true;
        }
        ...
        public bool ListMOTFWaits(float positionX, float rangeX, float positionY, float rangeY)
        {
            ...
            if (!this.IsListReady(1U))
                return false;
            RTC6Wrap.n_wait_for_encoder_in_range(this.Index + 1U, EncXmin, EncXmax, EncYmin, EncYmax);
            return true;
        }
        ...
        public bool ListMOTFEnd(Vector2 vPosition)
        {
            ...
            if (!this.IsListReady(1U))
                return false;
            RTC6Wrap.n_fly_return(this.Index + 1U, X, Y);
            this.vPhysical = vector2;
            this.vLogical = vPosition;
            return true;
        }
        ...
        public bool CtlCharacterBegin(uint asciiCode, CharacterSet characterSet = CharacterSet._0)
        {
            ...
            if (Char > 1023U)
            {
                Logger.Log(Logger.Type.Error, string.Format("rtc6 invalid character set= {0}. ascii code= {1}", (object)characterSet, (object)asciiCode), Array.Empty<object>());
                return false;
            }
            RTC6Wrap.n_load_char(this.Index + 1U, Char);
            return true;
        }
        ...
        {
            RTC6Wrap.n_list_return(this.Index + 1U);
            return true;
        }
        ...
        public bool CtlCharacterSetIsExist(uint asciiCode, CharacterSet characterSet = CharacterSet._0)
        {
            uint Char = (uint)characterSet * 256U + asciiCode;
            if (Char <= 1023U)
                return uint.MaxValue != RTC6Wrap.n_get_char_pointer(this.Index + 1U, Char);
            Logger.Log(Logger.Type.Error, string.Format("rtc6 invalid character set= {0}. ascii code= {1}", (object)characterSet, (object)asciiCode), Array.Empty<object>());
            return false;
        }
        ...
        {
            RTC6Wrap.n_set_serial_step(this.Index + 1U, serialNo, incrementStep);
            this.SerialStartNo = serialNo;
            this.SerialIncrementStep = incrementStep;
            Logger.Log(Logger.Type.Warn, string.Format("rtc6 [{0}]: reset to serial {1}, {2}", (object)this.Index, (object)serialNo, (object)incrementStep), Array.Empty<object>());
            return true;
        }
        ...
        public bool ListText(string text, CharacterSet characterSet = CharacterSet._0)
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(2U))
                return false;
            RTC6Wrap.n_select_char_set(this.Index + 1U, (uint)characterSet);
            RTC6Wrap.n_mark_text_abs(this.Index + 1U, text);
        ...
        public bool ListDate(DateFormat dateFormat, bool leadingWithZero, CharacterSet characterSet = CharacterSet._0)
        {
            if (this.CtlGetStatus(RtcStatus.Aborted) || !this.IsListReady(3U))
                return false;
            RTC6Wrap.n_time_fix(this.Index + 1U);
            RTC6Wrap.n_select_char_set(this.Index + 1U, (uint)characterSet);
            uint Mode = (uint)(0 | 2);
            if (leadingWithZero)
                Mode |= 1U;
            RTC6Wrap.n_mark_date_abs(this.Index + 1U, (uint)dateFormat, Mode);
            this.vPhysical = new Vector2(float.MaxValue, float.MaxValue);
            this.vLogical = new Vector2(float.MaxValue, float.MaxValue);
        ...
        public bool ListTime(TimeFormat timeFormat, bool leadingWithZero, CharacterSet characterSet = CharacterSet._0)
        {
            ...
            if (leadingWithZero)
                Mode |= 1U;
            RTC6Wrap.n_mark_time_abs(this.Index + 1U, (uint)timeFormat, Mode);
            this.vPhysical = new Vector2(float.MaxValue, float.MaxValue);
            this.vLogical = new Vector2(float.MaxValue, float.MaxValue);
        ...
        public bool ListSerial(uint numOfDigits, SerialFormat serialFormat, CharacterSet characterSet = CharacterSet._0)
        {
            ...
            if (numOfDigits > 12U)
            {
                Logger.Log(Logger.Type.Error, string.Format("rtc6 overflow serial number of digits= {0}", (object)numOfDigits), Array.Empty<object>());
                return false;
            }
            if (!this.IsListReady(2U))
                return false;
            ...
            RTC6Wrap.n_mark_serial_abs(this.Index + 1U, Mode, numOfDigits);
            this.vPhysical = new Vector2(float.MaxValue, float.MaxValue);
            this.vLogical = new Vector2(float.MaxValue, float.MaxValue);
