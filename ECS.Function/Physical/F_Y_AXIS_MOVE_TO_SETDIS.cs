...
namespace ECS.Function.Physical
{
    public class F_Y_AXIS_MOVE_TO_SETDIS : AbstractFunction
    {
        ...
        public override string Execute()
        {
            ...
            double startPostion = DataManager.Instance.GET_DOUBLE_DATA(IO_GET_Y_POSITION, out bool _);
            double setDistance = DataManager.Instance.GET_DOUBLE_DATA(VIO_DBL_Y_REL_DISTANCE, out bool _);
            double setVelocity = DataManager.Instance.GET_DOUBLE_DATA(VIO_DBL_Y_REL_VELOCITY, out bool _);

            _TargetPosition = startPostion + setDistance;

            DataManager.Instance.SET_DOUBLE_DATA(IO_DBL_Y_SET_DISTANCE, setDistance);
            DataManager.Instance.SET_DOUBLE_DATA(IO_DBL_Y_SET_VELOCITY, setVelocity);

            Thread.SpinWait(500);

            if (DataManager.Instance.SET_INT_DATA(IO_Y_MOVE_TO_SETDIS, 1))
            {
                //Thread.Sleep(1000);
        ...
        {
            DataManager.Instance.SET_DOUBLE_DATA(IO_GET_Y_POSITION, _TargetPosition);
        }
        public override void PostExecute()
        {
            IsAbort = false;
            IsProcessing = false;
            DataManager.Instance.SET_INT_DATA(IO_Y_MOVE_TO_SETDIS, 0);
        }
    }
}
